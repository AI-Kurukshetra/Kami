...
| 2 | Real-time Collaboration | must-have | Partial | Sharing/roles exist, but no live co-editing/presence.
| 3 | Document Upload & Management | must-have | Partial | Document CRUD exists, but no file upload pipeline for PDF/Word/PPT/images.
...
| 10 | Assignment Distribution | must-have | Partial | Assignment API foundation exists; full teacher-student distribution UX is pending.
...
| 14 | Search Functionality | important | Partial | Client-side document list filtering only; no full-text/indexed search.
| 15 | Classroom Management | must-have | Partial | Classroom API foundation exists; full roster and classroom UI workflow is pending.
...
| 19 | Notification System (email + in-app) | important | Partial | In-app document notifications implemented; email channel deferred.
...
| Notifications | Partial | In-app only.
...
- Password change flow includes client-side strength checks.

### Gaps to Fix
- Password policy aligned between signup and profile security update flow.
- Document create/update schemas now enforce trimmed non-empty content and title constraints.
...
1. File upload/storage + PDF render pipeline.
2. Annotation model + annotation APIs + annotation UI tools.
3. Real-time collaboration transport (presence/live cursors/live sync).
4. Comments/threading/mentions + notification extension.
5. Classroom + assignment distribution flows.
6. Export/print + search API + version history.
7. Cloud/LMS integrations.
